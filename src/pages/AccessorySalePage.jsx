...
export default function AccessorySalePage() {
    ...
    const fetchProducts = useCallback(async () => {
        ...
        try {
            ...
            if (selectedFilters.priceRanges && selectedFilters.priceRanges.length > 0) {
                ...
                const userMax = Math.max(...maxs);
                // Respect hard ceiling of 999? Actually if user selects "3000+" in filter it would conflict.
                // But FilterSidebar price ranges might not align with "Under 999" concept.
                // For now, allow intersection: filter min must be < 999 to show anything.
                // We'll let backend handle intersection of minPrice/maxPrice params.

                if (userMax !== Infinity) params.maxPrice = Math.min(999, userMax);
                else params.maxPrice = 999;
            }
            ...
            ['brand', 'color', 'fabric', 'occasion', 'fit', 'sizes'].forEach(key => {
                const stateKeyMap = {
                    'brand': 'brands',
                    'color': 'colors',
                    'fabric': 'fabrics',
            ...
            console.error("Failed to fetch accessory sale items", err);
        } finally {
            setLoading(false);
        }
            ...
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col gap-4 mb-8">
                    ...
                    <div className="flex justify-between items-end">
                        <div>
                            <h1 className="text-3xl md:text-4xl font-heading font-black text-brand-accent uppercase tracking-tight">Accessories Under ₹999</h1>
                            <p className="text-gray-500 text-sm mt-2">Complete your look without breaking the bank.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex-1">
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-20 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                                ...
                                <h3 className="text-xl font-bold text-gray-500">No accessories found.</h3>
                                    ...
                                    <div key={product._id} className="group bg-white rounded-lg overflow-hidden border border-transparent hover:border-black/5 hover:shadow-xl transition-all duration-300">
                                        ...
                                        <div className="p-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="font-bold text-brand-accent text-lg">₹{product.price}</span>
                                            </div>
                                            <button
                                                onClick={(e) => handleAddToCart(e, product)}
                                                className="w-full mt-3 bg-brand-charcoal text-white py-2 rounded font-bold text-sm hover:bg-black transition-colors"
                                            >
                                                Add to Cart
                                            </button>
                                        </div>
