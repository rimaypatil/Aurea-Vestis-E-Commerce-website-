    ...
    const fetchProducts = useCallback(async () => {
        ...
        try {
            ...
            if (selectedFilters.discountRanges && selectedFilters.discountRanges.length > 0) {
                ...
                const minDiscount = Math.min(...mins);
                const maxDiscount = Math.max(...maxs);

                // Backend expects 'discount[gte]' and 'discount[lte]' for ranges if possible, 
                // or we use the generic param handling if configured.
                // productController handles params like: discount[gte]=30
            ...
            console.error("Failed to fetch men's sale items", err);
        } finally {
            setLoading(false);
        }
            ...
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="flex flex-col gap-4 mb-8">
                    ...
                    <div className="flex justify-between items-end">
                        <div>
                            <h1 className="text-3xl md:text-4xl font-heading font-black text-red-600 uppercase tracking-tight">Men's 50% Off</h1>
                            <p className="text-gray-500 text-sm mt-2">Limited time offers on top men's fashion.</p>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div key={product._id} className="group bg-white rounded-lg overflow-hidden border border-transparent hover:border-black/5 hover:shadow-xl transition-all duration-300">
                                        ...
                                        <div className="p-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <span className="font-bold text-red-600 text-lg">₹{Math.floor(product.price * 0.5)}</span>
