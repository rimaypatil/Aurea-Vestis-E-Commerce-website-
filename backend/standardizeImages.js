...
const standardizeImages = async () => {
    try {
        ...
        for (const product of products) {
            ...
            // Data Integrity Fix: Ensure parentCategory exists
            ...
            // Fix invalid occasion values (Schema Enum: ['Casual', 'Formal', 'Party', 'Sports', 'Streetwear', 'None'])
            ...
            if (parentCat === 'sneakers' || cat === 'Sneakers') {
                ...
                // Check if it fits in accessories
                if (IMAGE_LIBRARY.accessories[cat]) {
                    ...
                    if (cat === 'Bags' || cat === 'Wallets' || cat === 'Belts') {
                        ...
                        // Default to men's miscellaneous if unsure, or specific logic?
                        // Let's safe default to men's default for now as it's often the base
                        library = IMAGE_LIBRARY.men;
                        categoryKey = 'default';
                    }
                }
            ...
            // If we found a string instead of array (unlikely with my structure but good safety)
            if (typeof targetImages === 'string') targetImages = [targetImages];

            // Pick an image deterministically based on name length to vary it slightly but consistently
